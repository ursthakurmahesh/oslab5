...
{
  // Remove the shared-memory mapping before freeing the page table.
  // The child does not own the physical page; only the process that
  // originally allocated it (shm_owner == 1) frees the physical page.
  if (p->shm_pa != 0 && p->pagetable != 0){
...
kfork(void)
{
  ...
  // If the parent has a shared-memory page, map the SAME physical page
  // into the child's address space. Normal uvmcopy() above does not copy
  // this page because SHMVA lies outside p->sz.
  if(p->shm_pa != 0){
    if(mappages(np->pagetable, SHMVA, PGSIZE,
